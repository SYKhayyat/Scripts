...
def is_org_level2_heading(line):
    """Check if line is an Org level 2 heading (starts with ** but not ***)."""
    ...
    for line in lines:
        ...
        if trimmed_line:
            if cleaned_lines and prev_was_heading and is_org_level2_heading(trimmed_line):
                ...
                if is_org_level2_heading(last_line.rstrip('\n')):
                    heading_text = trimmed_line[2:].lstrip()
                    if last_line.rstrip('\n').endswith(' '):
                        cleaned_lines[-1] = last_line.rstrip('\n') + heading_text + '\n'
                    else:
                        cleaned_lines[-1] = last_line.rstrip('\n') + ' ' + heading_text + '\n'
                else:
                ...
                cleaned_lines.append(trimmed_line + '\n')
        prev_was_heading = is_org_heading(line)
