...
if __name__ == "__main__":
    ...
    def handle_single_file(self, input_path: str, output_file: str):
        """Handle single file conversion with conflict resolution"""
        if not output_file:
            output_file = input_path.replace('.docx', '.org')
        
        conflict_mode = self.get_file_conflict_mode(output_file)
        ...
        # Handle single file
        if os.path.isfile(input_path) and input_path.lower().endswith('.docx'):
            self.handle_single_file(input_path, output_file)
        
        # Handle folder
        elif os.path.isdir(input_path):
            # For folder, always use the global conflict mode
